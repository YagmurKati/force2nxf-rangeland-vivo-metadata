#!/usr/bin/env python3
"""Collect VIVO-ready metadata for one FORCE2NXF-Rangeland run on Kubernetes."""
...
DEFAULT_WORKFLOW_NAME = "Long-term vegetation dynamics in the Mediterranean (FORCE2NXF)"
DEFAULT_WORKFLOW_URI = (
    "http://example.org/vivo-import/run-metadata/workflow/"
    "long-term-vegetation-dynamics-in-the-mediterranean-force2nxf"
)
DEFAULT_PUBLICATION_URI = (
    "http://141.20.184.157:8080/vivo/individual/"
    "a1-publication-key-lehmann2021forceon"
)
DEFAULT_CODE_URI = (
...
# note in the workflow-predicate builder). To credit Florian Katerndahl and
# Dirk Pflugmacher, add their VIVO person URIs to
# DEFAULT_RESPONSIBLE_RESEARCHER_URIS below.
DEFAULT_RESPONSIBLE_RESEARCHERS: List[str] = []
...
DEFAULT_LANGUAGE_URIS = [
    ...
    "http://example.org/vivo-import/run-metadata/language/r",
]
DEFAULT_LANGUAGE_LABELS = {
    "http://example.org/vivo-import/run-metadata/language/groovy": "Groovy / Nextflow DSL2",
    "http://example.org/vivo-import/run-metadata/language/shell": "Shell",
    "http://example.org/vivo-import/run-metadata/language/r": "R",
}
...
DEFAULT_OUTPUT_STEM = "force2nxf-rangeland-workflow-public-metadata"
...
DEFAULT_REMOTE_RESULTS_DIR = "/workspace/rangeland/results"
DEFAULT_REMOTE_DEBUG_LOG = (
    "/workspace/rangeland/FORCE2NXF-Rangeland/nextflowWF/.nextflow.log"
)
DEFAULT_REMOTE_CONSOLE_LOG = (
    "/workspace/rangeland/results/nextflow-rangeland-fixed.log"
)
...
CONTAINER_RE = re.compile(
    r"(?m)^\s*(?:process\.)?container\s*(?:=|\s)\s*['\"]([^'\"]+)['\"]"
)

DEFAULT_REMOTE_CODE_PATH = (
    "/workspace/rangeland/FORCE2NXF-Rangeland/nextflowWF"
)
DEFAULT_WORKFLOW_DESCRIPTION = (
    "Nextflow DSL2 workflow running FORCE lower-level preprocessing and "
    "higher-level rangeland vegetation analysis for Landsat time-series data "
    "on the FONDA Kubernetes cluster."
)
DEFAULT_INPUT_DATA_URI = (
    "http://example.org/vivo-import/run-metadata/dataset/"
    "fonda-b5-eo-01-rangeland-input"
)
DEFAULT_INPUT_DATA_NAME = "FONDA B5 EO-01 rangeland input dataset"
DEFAULT_INPUT_DATA_LOCATION = "/shared/b5/eo-01"
DEFAULT_INPUT_DATA_SUMMARY = (
    "Shared read-only FORCE2NXF-Rangeland input dataset containing Landsat "
    "Level 1 scenes, DEM, water vapour database, AOI vectors, and endmember "
    "data. Inventory checked 2026-07-30: 2,794 scenes; sensors LT04, LT05, "
    "LE07; date range 1984-04-16 to 2006-12-31; tiles 180036, 181035, "
    "181036, 182035, 182036, and 183035."
)
DEFAULT_WORKFLOW_PARAMETERS = [
    "inputdata=/workspace/rangeland/FORCE2NXF-Rangeland/inputdata",
    "outdata=/workspace/rangeland/outputdata",
    "forceVer=3.6.5",
    "useCPU=4",
]
...
def detect_latest_remote_trace(namespace: str, pod: str) -> str:
    script = (
        f"ls -1t {shlex.quote(DEFAULT_REMOTE_RESULTS_DIR)}/trace*.txt 2>/dev/null "
        "| head -n 1"
    )
    path = run_cmd(
    ...
    if not path:
        raise RuntimeError(
            f"No trace*.txt file found under {DEFAULT_REMOTE_RESULTS_DIR}"
        )
    return path
...
def derive_stage_status(
    slug: str, tasks: Sequence[TaskRecord], run_finished: bool
) -> str:
    status = derive_status(tasks, run_finished)
    status_counts = Counter(task.status for task in tasks)
    failed_attempts = status_counts["FAILED"] + status_counts["ERROR"]
    if slug == "result-validation" and run_finished and failed_attempts:
        return "Ignored validation warning"
    return status


def stage_description(
    slug: str, tasks: Sequence[TaskRecord], status: str
) -> Optional[str]:
    status_counts = Counter(task.status for task in tasks)
    failed_attempts = status_counts["FAILED"] + status_counts["ERROR"]
    if not failed_attempts:
        return None
    if slug == "result-validation" and status == "Ignored validation warning":
        return (
            "The final checkResults task exited non-zero, but the workflow "
            "marked this error as ignored and completed. The FORCE2NXF-Rangeland "
            "README notes that checkResults can fail when the Kubernetes run uses "
            "a different downloaded dataset; use --skipCheckResults to omit this "
            "reference comparison."
        )
    if status == "Succeeded with warnings":
        return (
            "Some task attempts in this stage exited unsuccessfully and were "
            "retried. Later attempts completed and the Nextflow session finished, "
            "so the failed trace rows are retry attempts rather than a terminal "
            "stage failure."
        )
    return None
...
def http_get_json(
    ...
    request_headers = {
        ...
        "User-Agent": "force2nxf-rangeland-vivo-metadata-collector/1.0",
    }
...
def classify_stage(task_name: str) -> Tuple[str, str]:
    if task_name.startswith("preprocessing:generateTileAllowList"):
        return "tile-allow-list-generation", "Tile allow-list generation"
    if task_name.startswith("preprocessing:generateAnalysisMask"):
        return "analysis-mask-generation", "Analysis mask generation"
    if task_name.startswith("preprocessing:preprocess"):
        return "level-2-preprocessing", "FORCE Level 2 preprocessing"
    if task_name.startswith("preprocessing:mergeBOA"):
        return "boa-merge", "BOA tile merge"
    if task_name.startswith("preprocessing:mergeQAI"):
        return "qai-merge", "QAI tile merge"
    if task_name.startswith("higherLevel:processHigherLevel"):
        return "higher-level-processing", "Higher-level vegetation processing"
    if task_name.startswith("higherLevel:processMosaic"):
        return "mosaic-generation", "Trend mosaic generation"
    if task_name.startswith("higherLevel:processPyramid"):
        return "pyramid-generation", "Trend pyramid generation"
    if task_name.startswith("checkResults"):
...
def remote_source_find_command() -> str:
    return (
        "find . -type f \\( "
        "-name '*.nf' -o -name '*.config' -o -name '*.groovy' -o "
        "-name '*.py' -o -name '*.R' -o -name '*.r' -o -name '*.sh' -o "
        "-name '*.json' -o -name '*.yaml' -o -name '*.yml' -o "
        "-name Dockerfile -o -name environment.yml "
        "\\) "
        "! -path './.git/*' "
        "! -path './.nextflow/*' "
        "! -path './results/*' "
...
REMOTE_SOURCE_WALKER = r"""
import hashlib
import os

allowed_suffixes = {
    ".nf", ".config", ".groovy", ".py", ".R", ".r", ".sh",
    ".json", ".yaml", ".yml",
}
allowed_names = {"Dockerfile", "environment.yml"}
ignored_dirs = {".git", ".nextflow", "__pycache__", "results", "work", "vivo_metadata"}
...
for root, dirs, names in os.walk("."):
    ...
    for name in names:
        ...
        if suffix in allowed_suffixes or name in allowed_names:
...
for rel in sorted(files):
    ...
    with open(rel, "rb") as handle:
        for chunk in iter(lambda: handle.read(1024 * 1024), b""):
            digest.update(chunk)
    print(f"{digest.hexdigest()}  ./{rel}")
"""


REMOTE_SOURCE_CATTER = r"""
import os
...
for rel in sorted(files):
    try:
        with open(rel, "r", encoding="utf-8", errors="replace") as handle:
            print(handle.read())
    except OSError:
        pass
"""
...
def remote_source_manifest(namespace: str, pod: str, path: str) -> str:
    script = (
        f"cd {shlex.quote(path)} || exit 1\n"
        f"python3 - <<'PY'\n{REMOTE_SOURCE_WALKER}\nPY"
    )
    manifest = run_cmd(
        ["kubectl", "-n", namespace, "exec", pod, "--", "sh", "-c", script]
    )
    if not manifest.strip():
        raise RuntimeError(f"No remote source files found under {path}")
    return manifest


def remote_sha256_source(namespace: str, pod: str, path: str) -> str:
    manifest = remote_source_manifest(namespace, pod, path)
    return hashlib.sha256(manifest.encode("utf-8")).hexdigest()


def remote_git_metadata(
    namespace: str,
    pod: str,
    path: str,
) -> Tuple[Optional[str], Optional[bool]]:
    script = (
        f"git -C {shlex.quote(path)} rev-parse HEAD 2>/dev/null\n"
        f"git -C {shlex.quote(path)} status --porcelain 2>/dev/null | wc -l"
    )
    try:
        output = run_cmd(
            ["kubectl", "-n", namespace, "exec", pod, "--", "sh", "-c", script]
...
def images_from_remote_code(namespace: str, pod: str, path: str) -> List[str]:
    script = (
        f"cd {shlex.quote(path)} || exit 1\n"
        f"python3 - <<'PY'\n{REMOTE_SOURCE_CATTER}\nPY"
    )
    try:
        text = run_cmd(
            ["kubectl", "-n", namespace, "exec", pod, "--", "sh", "-c", script]
...
def resolve_code_identity(
    args: argparse.Namespace,
) -> Tuple[str, Optional[str], Optional[bool], Optional[Path], bool]:
    if args.code_path:
        ...
        if code_path.exists():
            ...
            return code_version, git_commit, git_dirty, code_path, False
        print(
            f"WARNING: local code path does not exist, using remote code path: {code_path}",
            file=sys.stderr,
        )
    if not args.code_remote_path:
        raise RuntimeError(
            "No usable code path. Provide --code-path or --code-remote-path."
        )
    code_version = remote_sha256_source(
        args.namespace,
        args.driver_pod,
        args.code_remote_path,
    )
    git_commit, git_dirty = remote_git_metadata(
        args.namespace,
        args.driver_pod,
        args.code_remote_path,
    )
    return code_version, git_commit, git_dirty, None, True


def parameter_summary(parameters: Sequence[str]) -> str:
    return "; ".join(parameter.strip() for parameter in parameters if parameter.strip())
...
def build_ttl(
    ...
    run_finished = bool(log_metadata.get("finished"))
    ...
    for stage in stages:
        ...
        stage_status = derive_stage_status(
            stage["slug"], stage_tasks, run_finished
        )
        if cached_only and not args.include_cached_origin_metrics:
            stage_status = "Cached"
        stage_note = stage_description(
            stage["slug"], stage_tasks, stage_status
        )
        stage_pods = unique(
            task.pod_name
    ...
    # rm:WorkflowProcessRun class declaration also removed 2026-07-30 - the
    # class exists in VIVO's TBox.
    # NOTE 2026-07-30: inline declarations of rm:hasWorkflowProcess,
    # rm:isWorkflowProcessOf, rm:workflows and rm:hasWorkflow as generic
    # rdf:Property were REMOVED. Those properties are defined in VIVO's TBox;
    # re-declaring them here wrote duplicates into vitro-kb-2, which VIVO
    # rendered as extra/empty property sections. Do not re-add them.
    if not args.existing_workflow_run_only:
        add_resource(
            lines,
            ttl_uri(engine_uri),
            [
    ...
    if not args.existing_workflow_run_only:
        cluster_predicates.append(("rm:hasWorkflow", ttl_uri(workflow_uri)))
    add_resource(lines, ttl_uri(cluster_uri), cluster_predicates)

    if args.input_data_uri:
        input_data_predicates: List[Tuple[str, str]] = [
            ("rdf:type", "vivo:InformationResource"),
            ("rdf:type", "prov:Entity"),
            ("rdfs:label", ttl_label(args.input_data_name)),
            ("dcterms:title", ttl_label(args.input_data_name)),
            ("dcterms:description", ttl_literal(args.input_data_summary)),
            ("prov:atLocation", ttl_literal(args.input_data_location)),
            ("prov:wasUsedBy", ttl_uri(run_uri)),
        ]
        add_resource(
            lines,
            ttl_uri(args.input_data_uri),
            input_data_predicates,
        )

    if args.existing_workflow_run_only:
        workflow_predicates = [
            ("rm:hasRun", ttl_uri(run_uri)),
        ...
        workflow_predicates: List[Tuple[str, str]] = [
            ...
            ("dcterms:description", ttl_literal(args.workflow_description)),
        ...
        if args.backend_uri:
            workflow_predicates.append(("rm:backend", ttl_uri(args.backend_uri)))
        if args.trace_archive:
            workflow_predicates.append(
                ("rm:traceArchive", ttl_literal(args.trace_archive, "xsd:anyURI"))
            )
        # NOTE 2026-07-30: rm:responsibleResearcher is an OBJECT property in VIVO
        # (range Person). Literal names are no longer emitted - they created a
        # second, generic datatype property and an empty section on every workflow
        # page. Use --responsible-researcher-uri instead; any plain names passed
        # via --responsible-researcher are only warned about here.
        for researcher in responsible_researchers:
            print(
                "WARNING: ignoring literal responsible researcher "
                f"'{researcher}' - pass a person URI via --responsible-researcher-uri",
                file=sys.stderr,
            )
    ...
    run_predicates: List[Tuple[str, str]] = [
        ...
        (
            "rm:jobName",
            ttl_literal(log_metadata.get("run_name") or "force2nxf-rangeland"),
        ),
        (
            "rm:taskCount",
    ...
    if args.workflow_parameter:
        run_predicates.append(
            (
                "dcterms:description",
                ttl_literal(
                    "Workflow parameters: "
                    + parameter_summary(args.workflow_parameter)
                ),
            )
        )
    if args.input_data_uri:
        run_predicates.append(("prov:used", ttl_uri(args.input_data_uri)))
    if args.trace_archive:
        run_predicates.append(
            ("rm:traceArchive", ttl_literal(args.trace_archive, "xsd:anyURI"))
        )
    ...
    for stage in stage_records:
        ...
        if stage.get("description"):
            predicates.insert(
                ...
                (
                    "dcterms:description",
                    ttl_literal(stage["description"]),
                ),
            )
        elif stage["cached_only"] and not stage["cached_origin_metrics"]:
            predicates.insert(
                5,
                (
                    "dcterms:description",
                    ttl_literal(
                        "This workflow process is part of the logical "
                        "FORCE2NXF workflow. In this Nextflow session all "
                        "tasks in this stage were reused from the Nextflow "
                        "cache, so no Kubernetes pods were launched and no "
                        "current-run energy or carbon measurement is reported "
                        "for this stage."
                    ),
                ),
            )
        ...
        if stage["cached_only"] and not stage["cached_origin_metrics"]:
            predicates.extend(
                [
                    (
                        "rm:cpuTimeCalculationMethod",
                        ttl_literal(
                            "No CPU metrics are reported because all tasks in "
                            "this stage were reused from the Nextflow cache in "
                            "the current session."
                        ),
                    ),
                    (
                        "rm:memoryCalculationMethod",
                        ttl_literal(
                            "No memory metrics are reported because all tasks "
                            "in this stage were reused from the Nextflow cache "
                    ...
                    (
                        "rm:energyCalculationMethod",
                        ttl_literal(
                            "No energy metrics are reported because all tasks "
                            "in this stage were reused from the Nextflow cache "
                            "in the current session."
                        ),
                    ),
                    ...
                    (
                        "rm:carbonCalculationMethod",
                        ttl_literal(
                            "Stage carbon emissions are not reported because "
                            "no current-run energy was measured for cached "
                            "tasks."
                        ),
                    ),
                ]
    ...
    audit = {
        ...
        "code": {
            ...
            "git_dirty": git_dirty,
            "uri": args.code_uri,
            "local_path": args.code_path,
            "remote_path": args.code_remote_path,
            "hashed_remote": getattr(args, "code_hashed_remote", False),
        },
        "input_data": {
            "uri": args.input_data_uri,
            "name": args.input_data_name,
            "location": args.input_data_location,
            "summary": args.input_data_summary,
        },
        "workflow_parameters": list(args.workflow_parameter or []),
        "prometheus": {
            "url": args.prom_url,
        ...
        "stages": [
            {
                ...
                "succeeded_task_count": sum(
                    task.status in {"COMPLETED", "CACHED"}
                    for task in stage["tasks"]
                ),
                "failed_task_count": sum(
                    task.status in {"FAILED", "ERROR"}
                    for task in stage["tasks"]
                ),
                "aborted_task_count": sum(
                    task.status == "ABORTED" for task in stage["tasks"]
                ),
                "executed_task_count": sum(
...
def build_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and Nextflow metadata for one FORCE2NXF-Rangeland run and write "
            "VIVO-ready TTL."
        )
    )
    ...
    parser.add_argument(
        "--debug-log-remote",
        default=DEFAULT_REMOTE_DEBUG_LOG,
    )
    parser.add_argument("--console-log-file", default=None)
    parser.add_argument("--console-log-remote", default=DEFAULT_REMOTE_CONSOLE_LOG)
    parser.add_argument(
        "--trace-timezone",
        default="UTC",
    ...
    parser.add_argument(
        "--code-path",
        default=None,
        help="Local FORCE workflow code path. If omitted, --code-remote-path is used.",
    )
    parser.add_argument(
        "--code-remote-path",
        default=DEFAULT_REMOTE_CODE_PATH,
        help="FORCE workflow code path inside the driver pod for code hashing.",
    )
    ...
    parser.add_argument("--input-data-uri", default=DEFAULT_INPUT_DATA_URI)
    parser.add_argument("--input-data-name", default=DEFAULT_INPUT_DATA_NAME)
    parser.add_argument("--input-data-location", default=DEFAULT_INPUT_DATA_LOCATION)
    parser.add_argument("--input-data-summary", default=DEFAULT_INPUT_DATA_SUMMARY)
    parser.add_argument(
        "--workflow-parameter",
        action="append",
        default=None,
        help=(
            "Repeat for each FORCE/Nextflow parameter to record on the run. "
            "Defaults match the current rangeland command."
        ),
    )
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Repeat for each literal researcher name; object URIs are preferred.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Repeat for each non-workflow-engine language URI; defaults to Shell and R.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Link this run to an existing VIVO workflow page without "
            "re-emitting workflow-level title, description, people, "
            "subproject, code, engine, backend, trace, or language metadata."
        ),
    )
    ...
    parser.add_argument(
        "--output-dir",
        default=str(here / "metadata" / "generated"),
    )
...
def main() -> None:
    ...
    if args.workflow_parameter is None:
        args.workflow_parameter = list(DEFAULT_WORKFLOW_PARAMETERS)
    code_version, git_commit, git_dirty, code_path, code_hashed_remote = (
        resolve_code_identity(args)
    )
    args.code_hashed_remote = code_hashed_remote
    images = unique(
        image
        for metrics in pod_metrics.values()
        for image in metrics.images
    )
    if not images and code_path is not None:
        images = images_from_code(code_path)
    if not images and args.code_remote_path:
        images = images_from_remote_code(
            args.namespace,
            args.driver_pod,
            args.code_remote_path,
        )
